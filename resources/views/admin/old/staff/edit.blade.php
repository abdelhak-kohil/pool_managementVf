...
<div class="bg-white shadow rounded-lg p-6 max-w-3xl mx-auto">
  <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Staff Member</h2>
  ...
  <form action="{{ route('staff.update', $staff->staff_id) }}" method="POST" class="space-y-5">
    @csrf
    <div>
    ...
    <div>
      ...
      <select name="role_id" required
        ...
        @foreach ($roles as $role)
          <option value="{{ $role->role_id }}" @selected($staff->role_id == $role->role_id)>
    ...
    <div>
      <label class="inline-flex items-center">
        <input type="checkbox" name="is_active" class="rounded text-blue-600 mr-2" {{ $staff->is_active ? 'checked' : '' }}>
