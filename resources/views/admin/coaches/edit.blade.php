...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Modifier Coach</h1>
            <p class="mt-2 text-sm text-gray-600">Mise à jour des informations de <span class="font-semibold">{{ $coach->first_name }} {{ $coach->last_name }}</span>.</p>
        </div>
    ...
    <form action="{{ route('coaches.update', $coach->staff_id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                        ...
                        <div class="flex items-center">
                            <label class="flex items-center cursor-pointer">
                                <span class="mr-3 text-sm font-medium text-gray-700">Compte Actif</span>
                                <div class="relative">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $coach->is_active ? 'checked' : '' }}>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-1">
                            ...
                            <input type="text" name="first_name" value="{{ old('first_name', $coach->first_name) }}" required 
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="last_name" value="{{ old('last_name', $coach->last_name) }}" required 
                                class="w-full h-11 px-4 rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 transition-shadow">
                        </div>
                        <div class="col-span-1">
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="phone_number" value="{{ old('phone_number', $coach->phone_number) }}" 
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-1">
                            ...
                            <input type="text" name="specialty" value="{{ old('specialty', $coach->specialty) }}" 
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="date" name="hiring_date" value="{{ old('hiring_date', $coach->hiring_date ? \Carbon\Carbon::parse($coach->hiring_date)->format('Y-m-d') : '') }}" 
                                class="w-full h-11 px-4 rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 transition-shadow">
                        </div>
                        <div class="col-span-full">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <select id="badge_select" class="w-full h-11 px-4 rounded-xl border-gray-300 focus:border-purple-500 focus:ring-purple-500 transition-shadow bg-white text-gray-700 disabled:bg-gray-100 disabled:text-gray-400"
                                        ...
                                        @if(isset($availableBadges) && $availableBadges->count() > 0)
                                            <option value="">— Changer pour un badge disponible —</option>
                                            @foreach($availableBadges as $badge)
                                                <option value="{{ $badge->badge_uid }}">Badge #{{ $badge->badge_uid }}</option>
                                            @endforeach
                                        @else
                                            <option value="">Aucun autre badge disponible</option>
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="relative flex items-center">
                                    ...
                                    <span class="flex-shrink-0 mx-4 text-gray-400 text-sm">OU MODIFIER MANUELLEMENT</span>
                                    <div class="flex-grow border-t border-gray-200"></div>
                                </div>
                                ...
                                <div class="flex gap-2">
                                    <div class="relative flex-grow">
                                        ...
                                        <input type="text" id="badge_uid" name="badge_uid" value="{{ old('badge_uid', $coach->badges->first()->badge_uid ?? '') }}" 
                                ...
                                <p class="text-xs text-gray-500">Laissez vide pour retirer le badge actuel.</p>
                            </div>
                        </div>
                                ...
                                if(badgeSelect && badgeInput) {
                                    badgeSelect.addEventListener('change', function() {
                                        ...
                                        if(selectedValue) {
                                            ...
                                            // If they select default option, do we clear? 
                                            // Maybe better not to clear if they just clicked by mistake, but let's stick to standard behavior
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <textarea name="notes" rows="4" 
                            class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 transition-shadow" placeholder="Information supplémentaire concernant ce coach...">{{ old('notes', $coach->notes) }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <div class="relative">
                                <select name="salary_type" class="w-full h-11 pl-4 pr-10 rounded-xl border-gray-300 focus:border-green-500 focus:ring-green-500 transition-shadow appearance-none bg-none">
                                    <option value="per_hour" {{ old('salary_type', $coach->salary_type) == 'per_hour' ? 'selected' : '' }}>Par Heure</option>
                                    <option value="per_session" {{ old('salary_type', $coach->salary_type) == 'per_session' ? 'selected' : '' }}>Par Séance</option>
                                    <option value="fixed" {{ old('salary_type', $coach->salary_type) == 'fixed' ? 'selected' : '' }}>Fixe (Mensuel)</option>
                                </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" name="hourly_rate" value="{{ old('hourly_rate', $coach->hourly_rate) }}" required 
