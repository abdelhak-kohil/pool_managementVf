  ...
  <div class="bg-white rounded-xl shadow overflow-x-auto border border-gray-100">
      <table class="min-w-full text-left text-gray-800">
        <thead class="bg-gray-50 border-b">
            ...
            <th class="py-3 px-4 font-medium">Role Name</th>
            <th class="py-3 px-4 font-medium text-right">Actions</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            @foreach($roles as $role)
            <tr class="hover:bg-blue-50 transition" x-show="matchesSearch('{{ strtolower($role->role_name) }}')">
                <td class="py-3 px-4 text-gray-500">{{ $role->role_id }}</td>
                <td class="py-3 px-4 font-medium">{{ $role->role_name }}</td>
                <td class="py-3 px-4 text-right">
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('roles.edit', $role->role_id) }}" class="text-yellow-600 hover:text-yellow-800 font-medium">✏️ Edit</a>
                        <a href="{{ route('roles.permissions', $role->role_id) }}" class="text-blue-600 hover:text-blue-800 font-medium">🔒 Permissions</a>
                        <form action="{{ route('roles.destroy', $role->role_id) }}" method="POST" class="delete-form inline-block">
                            @csrf @method('DELETE')
