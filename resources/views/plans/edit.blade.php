...
<div class="space-y-8">
  ...
  <div class="bg-white shadow rounded-xl border border-gray-100 p-8 max-w-4xl mx-auto">
    <form method="POST" action="{{ isset($plan) ? route('plans.update', $plan->plan_id) : route('plans.store') }}" class="space-y-10">
      @csrf
      ...
      <div class="pt-6 border-t flex justify-end gap-4">
        <a href="{{ route('plans.index') }}"
           class="px-5 py-2 rounded-lg border border-gray-300 hover:bg-gray-100 transition">Annuler</a>
        <button type="submit"
