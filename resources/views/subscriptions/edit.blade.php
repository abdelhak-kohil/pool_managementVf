...
<form action="{{ route('subscriptions.update', $subscription->subscription_id) }}" method="POST" class="space-y-8">
  ...
  <div class="bg-white rounded-xl shadow border border-gray-100 p-6">
    <h2 class="text-xl font-semibold text-blue-700 mb-4 flex items-center gap-2">🧍‍♂️ Informations du Membre</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <span class="inline-block px-3 py-2 rounded-lg text-sm font-semibold
          @if(optional($subscription->member->accessbadge)->status === 'active') bg-green-100 text-green-700
          @elseif(optional($subscription->member->accessbadge)->status === 'inactive') bg-gray-100 text-gray-700
          @elseif(optional($subscription->member->accessbadge)->status === 'lost') bg-yellow-100 text-yellow-700
          @elseif(optional($subscription->member->accessbadge)->status === 'revoked') bg-red-100 text-red-700
          @else bg-gray-200 text-gray-600 @endif">
          {{ ucfirst(optional($subscription->member->accessbadge)->status ?? 'Non défini') }}
        </span>
      </div>
    </div>
  </div>
